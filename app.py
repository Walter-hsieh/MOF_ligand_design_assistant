...
class SmilesToImageWindow(ctk.CTkToplevel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.results_data = []
        # FIX 1 of 3: Initialize a list to hold image references
    ...
    def run_generation(self):
        ...
        # FIX 2 of 3: Clear the reference list before generating new images
        ...
        for result in self.results_data:
            ...
            if result['pil_image']:
                ...
                # FIX 3 of 3: Add the image object to our reference list
...
# The main App class and its methods are unchanged. You can use your existing code.
# The code below is provided just for completeness of the file.
class ExplanationWindow(ctk.CTkToplevel):
    """A popup window to display AI-generated explanations."""
...
class App(ctk.CTk):
    ...
    def copy_to_clipboard(self, text_to_copy: str, button: ctk.CTkButton):
        try:
            ...
            messagebox.showerror("Clipboard Error", f"Could not copy text to clipboard: {e}")
    def request_explanation(self, candidate_data: dict):
        ...
        if ligand_name in self.explanation_windows and self.explanation_windows[ligand_name].winfo_exists():
            self.explanation_windows[ligand_name].focus()
            return
        exp_window = ExplanationWindow(ligand_name=ligand_name, master=self)
        self.explanation_windows[ligand_name] = exp_window
        exp_window.focus()
        thread = threading.Thread(target=self.run_explanation_thread, args=(candidate_data, exp_window), daemon=True)
        thread.start()
    def run_explanation_thread(self, candidate_data: dict, window: ExplanationWindow):
        ...
        ligand_name = candidate_data.get("name")
        ligand_smiles = candidate_data.get("smiles")
        explanation_text = backend.get_explanation_for_ligand(original_prompt=original_prompt, ligand_name=ligand_name, ligand_smiles=ligand_smiles, vector_store_to_use=self.vector_store)
    ...
    def create_ligand_card(self, parent, candidate_data):
        ...
        explanation_btn = ctk.CTkButton(btn_frame, text="💡 Explain", command=lambda d=candidate_data: self.request_explanation(d))
        explanation_btn.grid(row=0, column=3, padx=2)
        return card
